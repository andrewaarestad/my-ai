...
4. **Run database migrations**:
    ...
    # Apply the latest schema changes to your database
    pnpm db:push
    ```
...
# Push the current schema directly to the database (development convenience)
pnpm db:push

# Apply any pending SQL migrations (production-safe)
pnpm db:migrate
...
| `pnpm db:generate` | Create SQL migrations from the Drizzle schema |
| `pnpm db:push` | Apply the current schema directly to the database |
